...
// Vercel serverless functions have no writable local disk.
// All CSV content is fetched and returned as strings in memory.
// ============================================================
...
export interface DownloadResult {
  filename: string;
  csvContent: string; // raw CSV text held in memory
  sourceType: SourceType;
  fileDate: string | null;
}
...
/**
 * Discover and download all new CSV files from both county directories.
 * Returns raw CSV content in memory — no disk I/O.
 */
export async function downloadNewFiles(): Promise<DownloadResult[]> {
  ...
  console.log(
    `[Downloader] ${newFiles.length} new file(s) of ${allFiles.length} discovered.`
  );
  ...
  for (const file of newFiles) {
    try {
      ...
      console.log(
        `[Downloader] Fetched ${csvContent.length} bytes: ${file.filename}`
      );
    } catch (err) {
